...
export function composePreviewPrompt(
  ...
  const parts: string[] = [
    "Pure black and white line art coloring book character. ONLY black lines on white background. NO color, NO shading, NO gradients, NO gray tones, NO fills. Bold clean outlines only. Single character, one character only.",
...
export function composePagePrompt(
  ...
  // Lead with SCENE and BACKGROUND so the model prioritizes environment (coloring-book fun).
  // Style + theme first, then full scene/background block, then character.
  const parts: string[] = [
    "Pure black and white line art coloring book page. ONLY black lines on white background. NO color, NO shading, NO gradients, NO gray tones, NO fills, NO painted areas. Bold clean outlines only.",
    `Theme: ${manifest.theme}.`,
    "",
    "SCENE AND BACKGROUND (critical for coloring—must be as detailed as the character):",
    `Full scene: ${scene}.`,
    "The background and environment are essential: include foreground elements, midground details, and background layers. Fill the page with theme-appropriate setting, objects, patterns, and decorative details that a child can color. Do not leave the background empty or minimal.",
    `All background elements must be strictly from the theme (${manifest.theme}). No unrelated objects (e.g. no castles, balloons, or generic fantasy elements unless the theme explicitly includes them).`,
  ];

  if (backgroundStr) {
    parts.push(
      `Specific background elements to include: ${backgroundStr}.`,
    );
  }
  ...
  // Character block
  parts.push(
    `CHARACTER (same character every page): THE EXACT SAME CHARACTER named ${manifest.characterName}. ONE HEAD ONLY. Single character, single body. Maintain IDENTICAL character appearance and consistency with previous pages.`,
  );
